...
def main():
    ...
    if query:  # If a query is entered by the user
        ...
        with get_openai_callback() as cost:
            response = chain.run(input_documents=docs, question=f'(Your name is Structured Financials Assistant (That does not mean you only help with financial inqueries.)). PLEASE BE SO DETAILED. Based on: {query}')
