...
def process_file(file, file_type, query):
    ...
    if file_type == 'application/pdf':  # For PDF files
        ...
        message = f'Digging deep into your {pages_length} page(s)... \
            ETA {int(eta_minutes)} minute(s) and {int(eta_seconds)} second(s).' \
                if not int(eta_minutes) == 0 else \
                    f'Digging deep into your {pages_length} page(s)... \
                    ETA {int(eta_seconds)} second(s).'
